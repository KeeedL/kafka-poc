...
public class DefaultAccountService implements AccountService {
    ...
    @Override
    @Transactional // FUTURE: Should lock rows during transaction
    ...
    // FUTURE: Should trace and log bidirectionally
    private Optional<AccountDto> updateAccountBalance(final String accountUuid, final float amount, boolean isDeposit) {
        return repository.findByUuid(accountUuid).map(account -> {
            float newAmount = isDeposit ? depositProcessor.processDepositAmount(account.getAmount(), amount)
